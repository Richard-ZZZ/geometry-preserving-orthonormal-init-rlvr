...
# RLPO for verl DAPO experiments.
# Initialization: A = orthonormal principal SVD directions (Vh[:r]), B = 0.
...
SEED=${SEED:-42}
EXP_NAME=${EXP_NAME:-rlpo_1p5b_paper_seed${SEED}}
LOG_FILE=${LOG_FILE:-./outputs/${EXP_NAME}.log}

PROJECT_NAME=${PROJECT_NAME:-RLPO-Paper-Replication}
...
# RLPO initialization does NOT modify the base model (delta=0).
# Use original pretrained weights + orthonormal-initialized adapter.
MODEL_BASE_PATH=${MODEL_BASE_PATH:-${PRETRAINED}}
LORA_ADAPTER_PATH=${LORA_ADAPTER_PATH:-${BUNDLE_BASE}/rlpo_r16_a32/adapter}
