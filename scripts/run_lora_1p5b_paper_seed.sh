...
# Paper-aligned LoRA baseline (1.5B) for verl DAPO experiments.
# Expected env vars per run:
#   SEED, EXP_NAME, LOG_FILE

SEED=${SEED:-42}
EXP_NAME=${EXP_NAME:-lora_1p5b_paper_seed${SEED}_r16_a32_lr1e5}
LOG_FILE=${LOG_FILE:-./outputs/${EXP_NAME}.log}

PROJECT_NAME=${PROJECT_NAME:-LoRA-Baseline-Paper-Replication}
TRAIN_FILE=${TRAIN_FILE:-./data/dapo-math-17k.parquet}
TEST_FILE=${TEST_FILE:-./data/aime-2024.parquet}
MODEL_PATH=${MODEL_PATH:-${MODEL_BASE_PATH:-deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B}}

LORA_RANK=${LORA_RANK:-16}
LORA_ALPHA=${LORA_ALPHA:-32}
LORA_ADAPTER_PATH=${LORA_ADAPTER_PATH:-}
TARGET_MODULES=${TARGET_MODULES:-all-linear}
LORA_A_INIT_METHOD=${LORA_A_INIT_METHOD:-default}
LORA_A_WAVELET=${LORA_A_WAVELET:-haar}
...
CMD=(
  ...
  +actor_rollout_ref.model.lora_A_init_method=${LORA_A_INIT_METHOD} \
  +actor_rollout_ref.model.lora_A_wavelet=${LORA_A_WAVELET} \
  ...
  +trainer.log_base_model_kl=True \
